...
if $cygwin ; then
    ...
    SEP=""; for dir in $ROOTDIRSRAW ; do ROOTDIRS="$ROOTDIRS$SEP$dir"; SEP="|"; done
    OURCYGPATTERN="(^($ROOTDIRS))"
    if [ "$GRADLE_CYGPATTERN" != "" ] ; then OURCYGPATTERN="$OURCYGPATTERN|($GRADLE_CYGPATTERN)"; fi
    if [ "$OURCYGPATTERN" = "" ] ; then CCAT=find; else CCAT="find -L"; fi
    for arg in "$@" ; do
        if echo $arg | $CCAT $OURCYGPATTERN >/dev/null 2>&1 ; then
            `echo $arg | cygpath --path --ignore --mixed -`
        else
            echo $arg
        fi
    done
